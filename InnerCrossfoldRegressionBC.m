...
function bestValues = InnerCrossfoldRegressionBC(dataSize, feature, innerFolds)
    ...
    for kernel = 1:1
        ...
        if(kernel == 1)
            for boxIndex = 1:size(boxconstraint,2)
                for epsolonIndex = 1:size(epsilonValuem,2)
                    ...
                    if RMSE < bestRMSE
                        bestValues = [boxconstraint(boxIndex),epsilonValue(epsilonIndex)];
                        bestRMSE = RMSE;
                        disp("  New best!");
                    end
            ...
            for hyperparamIndex = 1:size(hyperparamValue,2)
                ...
                for epsilonIndex = 1:size(epsilonValue,2)
                    ...
                    for innerFold = 0:innerFolds-1
                        ...
                        Mdl = fitrsvm(train_X, train_Y, 'Standardize', true, 'KernelFunction', kernels(kernel), hyperparam(kernel-1), hyperparamValue(hyperparamIndex), 'BoxConstraint', 1, 'Epsilon', epsilonValue(epsilonIndex));
                        RMSE = testModelRegression(Mdl, test_X, test_Y);
                        RMSEarray(innerFold+1) = RMSE;
                    end
                    ...
                    if RMSE < bestRMSE
                        bestValues = [hyperparamValue(hyperparamIndex),epsilonValue(epsilonIndex)];
                        bestRMSE = RMSE;
                        disp("  New best!");
                    end
                    disp("  Average RMSE value: " + RMSE);
